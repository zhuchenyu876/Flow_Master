...
class GoogleConvertRequest(BaseModel):
    ...
    file_path: str = Field(..., description="Dialogflow CX 导出的 flow JSON 文件路径")
    ...
    model_config = ConfigDict(
        ...
        json_schema_extra={
            "example": {
                "record_id": 123,
                "file_path": "input/exported_flow_xxx.json",
...
# Dyna.ai API 基础 URL（所有接口基于此 URL 拼接）
DYNA_API_BASE = os.getenv("DYNA_API_BASE", "https://agents.dyna.ai")

# 回调接口URL（基于 DYNA_API_BASE 拼接）
CALLBACK_URL = os.getenv("CALLBACK_URL", f"{DYNA_API_BASE}/openapi/v2/robot/convert/callback")

# Agent 上传接口 URL
DYNA_API_BASE_URL = f"{DYNA_API_BASE}/openapi/v1/agent/"
...
app = FastAPI(
    title="Dialogflow CX 迁移工具 API",
    description="""
## 功能说明

将 Dialogflow CX 的 Agent 迁移到 Dyna.ai 平台。
...
1. 接收请求，创建任务
2. 后台执行迁移（提取数据 → 转换 → 创建知识库 → 生成Agent → 上传）
3. 完成后回调通知
    """,
    version="2.0.0",
    docs_url="/docs",
...
async def create_convert_task(
    ...
    try:
        ...
        try:
            ...
            logger.debug(f"✅ 数据库记录创建成功: task_id={task_id}")
...
def callback_result(request: GoogleConvertRequest, status: int, res_id: str, error_message: str = ""):
    ...
    # 构建回调请求
    headers = {
        "cybertron-robot-key": request.cybertron_robot_key,
        "cybertron-robot-token": request.cybertron_robot_token,
        "username": request.username,
    ...
    logger.debug(f"📤 回调通知: {CALLBACK_URL}")
...
def run_migration_with_callback(
    ...
    # 语言映射：接口传入 zh -> 内部使用 zh-cn
    lang = request.language
    
    # 使用 task_id 作为输出目录，避免不同任务之间文件覆盖
    ...
    try:
        ...
        # 先验证文件格式
        is_valid, error_msg = validate_dialogflow_cx_file(request.file_path)
        if not is_valid:
        ...
        if not success:
            error_msg = "Step 0 提取数据失败：文件结构不符合预期"
        ...
        if os.path.exists(intents_lang_file):
            ...
            logger.info(f"   - ✅ 已加载 intents_mapping: {len(intents_mapping)} 个意图")
            logger.info(f"   - ✅ 已加载 intents_training_phrases: {len(intents_training_phrases)} 个意图的训练短语")
            # 打印前3个训练短语示例
            for i, (name, phrases) in enumerate(list(intents_training_phrases.items())[:3]):
                logger.debug(f"      - {name}: {len(phrases)} 个训练短语")
        else:
            logger.warning(f"   - ⚠️ intents_lang_file 不存在: {intents_lang_file}")
            logger.warning(f"   - ⚠️ intents_training_phrases 为空，版本2的语义判断节点将没有训练短语！")
        
        logger.info(f"🔄 Step 2: 开始转换 workflow...")
        logger.info(f"   - Intents 文件: {intents_lang_file}")
        logger.info(f"   - Fulfillments 文件: {fulfillments_lang_file}")
        logger.info(f"   - 语言: {lang}")
        
        intent_parameters_file = os.path.join(step1_dir, 'intent_parameters.json')
        logger.info(f"   - Intent parameters 文件: {intent_parameters_file}")
        logger.info(f"   - 文件存在: {os.path.exists(intent_parameters_file)}")
        ...
        logger.info(f"   - 创建 WorkflowConverter...")
        logger.debug(f"   - intents_mapping 数量: {len(intents_mapping)}")
        logger.debug(f"   - intent_parameters_file: {intent_parameters_file}")
        logger.debug(f"   - language: {lang}")
        
        # write by senlin.deng 2026-01-23
        # 可配置参数
        global_config = {
        ...
        # write by senlin.deng 2026-01-04
        # 意图识别版本2：增加使用语义判断节点进行意图识别
        ...
        if intent_recognition_version not in ['Knowledge Base Version', 'Semantic Judgement Version']:
            logger.error(f"faq_version 参数: {intent_recognition_version} 不支持")
        ...
        try:
            ...
            logger.info(f"   - ✅ WorkflowConverter 已创建，开始转换...")
            ...
            logger.error(f"   - ❌ {error_msg}")
            logger.error(f"   - 文件: {intent_parameters_file}")
            error_detail = traceback.format_exc()
            logger.error(f"   - 错误详情:\n{error_detail}")
            raise
        except Exception as e:
            ...
            logger.error(f"   - 错误详情:\n{error_detail}")
            raise
        # write by senlin.deng 2026-01-06
        ...
        generated_workflows = converter.convert_to_multiple_workflows(
            ...
            flow_file=request.file_path,
            lang=lang,
            output_dir=step2_dir,
            entities_file=entities_lang_file if os.path.exists(entities_lang_file) else None
        )
        
        logger.info(f"✅ Step 2 完成: 生成了 {len(generated_workflows)} 个 workflow")
        ...
        if intent_recognition_version == 1:
            ...
            logger.info("="*80)
            logger.info("Step 3.0: 检查数据库中的知识库映射")
            logger.info("="*80)
            ...
            try:
                ...
                if reused_count > 0:
                    logger.debug("")
                    logger.debug("="*80)
                    logger.debug("🎯 发现数据库中已有知识库映射！")
                    logger.debug("="*80)
                    logger.debug(f"✅ 复用知识库: {reused_count} 个")
                    logger.debug(f"🆕 需要新建: {new_count} 个")
                    logger.debug(f"📊 总计: {total_intents} 个意图")
                    logger.debug("")
                    logger.debug("💡 复用说明:")
                    logger.debug(f"   - 这些知识库在之前的任务中已创建（相同 robot_key + intent + language）")
                    logger.debug(f"   - 本次任务将直接使用已有的知识库ID，无需重新创建")
                    logger.debug(f"   - 映射关系已保存到数据库和本地JSON文件")
                    logger.debug("="*80)
                elif total_intents > 0:
                    logger.debug(f"   ℹ️  未找到可复用的知识库映射，将创建 {new_count} 个新知识库")
                else:
                    logger.warning(f"   ⚠️  未找到意图文件或意图文件为空，无法进行知识库复用检查")
                    logger.warning(f"   意图文件路径: {intents_file}")
            except Exception as e:
                logger.warning(f"   ⚠️  知识库复用检查失败: {e}")
                logger.warning(f"   将跳过复用检查，继续执行 step3")
                # 如果检查失败，设置默认值
                reused_count, new_count = 0, 0
            ...
            if new_count == 0 and reused_count > 0:
                logger.debug("")
                logger.debug("⏭️  所有知识库均已复用，跳过创建步骤，无需上传")
                logger.debug("")
            ...
            # 例如：exported_flow_AccountServicing_FAQ.json -> AccountServicing_FAQ
            exported_flow_basename = os.path.basename(request.file_path)
            agent_name = ""
            ...
            logger.info(f"📋 使用 agent 名称作为知识库后缀: {agent_name}")

            # 设置环境变量（包括输出目录，与 task_id 关联），控制step3执行是否跳过创建kb知识库，不使用.env文件
            ...
            # 设置 step3 使用的目录，与当前任务的 output_dir 一致
            ...
            # 确保 step3 使用与当前任务一致的语言（根据用户输入 language 映射后的 lang）
            os.environ['STEP_3_LANGUAGE'] = lang
            # 使用 agent 名称作为知识库后缀
            ...
            try:
                ...
                # 传入 task_id 和 db，优先从数据库读取映射
                # 如果step3卡住，可以临时跳过workflow更新步骤
                skip_workflow_update = os.getenv("SKIP_STEP3_WORKFLOW_UPDATE", "True").lower() == "true"  # 默认改为True
                if skip_workflow_update:
                    logger.warning("⚠️  检测到 SKIP_STEP3_WORKFLOW_UPDATE=True，跳过 step3 workflow 更新")
                    # 只执行知识库创建，不执行workflow更新
                    logger.info("🔄 调用 step3_kb_workflow.step1_create_knowledge_bases...")
                    success = step3_kb_workflow.step1_create_knowledge_bases(task_id=task_id, db_session=db)
                    logger.info(f"📊 Step 3 返回值: success={success}")
                else:
                ...
                if success:
                    ...
                    logger.error(f"❌ Step 3 失败，终止流程")
                    raise Exception(error_msg)
            except Exception as e:
                error_msg = f"知识库创建失败: {str(e)}"
                crud.update_step_status(db, task_id, 3, StepStatus.FAILED, error_msg)
                crud.create_error(db, task_id, str(e), step_number=3, error_type=type(e).__name__)
                logger.error(f"❌ Step 3 异常: {e}")
                # 如果是workflow更新卡住，可以选择继续执行后续步骤
                if os.getenv("CONTINUE_ON_STEP3_ERROR", "False").lower() == "true":
                    logger.warning("⚠️  检测到 CONTINUE_ON_STEP3_ERROR=True，即使step3失败也继续执行")
                    crud.update_step_status(db, task_id, 3, StepStatus.COMPLETED, "知识库创建完成（跳过workflow更新）")
                else:
                    raise  # 中断后续流程
        else:
            logger.info("🔄 Step 3: 语义判断节点版本，无需创建知识库，直接跳过")
        # ========================================
        # Step 4: 提取变量
        # ========================================
        # 确保 generated_workflows 变量存在
        if 'generated_workflows' not in locals() and 'generated_workflows' not in globals():
            logger.error("❌ generated_workflows 变量未定义，无法继续 Step 4")
            raise ValueError("generated_workflows 变量未定义")
        
        logger.info(f"🔄 开始 Step 4: 变量提取 ({len(generated_workflows)} 个 workflow)")
        logger.info(f"📁 Step 2目录: {step2_dir}")
        logger.info(f"📁 Step 4目录: {os.path.join(output_dir, 'step4_variables', lang)}")
        
        logger.info("💾 正在更新数据库状态...")
        try:
            crud.update_step_status(db, task_id, 4, StepStatus.RUNNING, "正在提取变量...")
            logger.info("✅ 数据库状态更新完成")
        except Exception as e:
            logger.warning(f"⚠️  更新数据库状态失败，继续执行: {e}")
            # 不中断流程，继续执行

        logger.info("📦 正在导入 step4_extract_variables 模块...")
        try:
            import importlib
        ...
        logger.info(f"📁 正在创建 Step 4 输出目录: {step4_dir}")
        os.makedirs(step4_dir, exist_ok=True)
        logger.info(f"✅ Step 4 输出目录已创建")

        logger.info(f"📋 Generated workflows 列表: {generated_workflows}")
        logger.info(f"📊 共 {len(generated_workflows)} 个 workflow 需要处理")
        logger.info(f"🔄 开始处理 {len(generated_workflows)} 个 workflow...")
        processed_count = 0
        for wf_name in generated_workflows:
            logger.info(f"🔄 处理 workflow: {wf_name}")
            ...
            logger.info(f"🔍 检查文件: {nodes_file}")

            if os.path.exists(nodes_file):
                logger.info(f"✅ 文件存在，开始处理: {wf_name}")
                try:
                    with open(nodes_file, 'r', encoding='utf-8') as f:
                        nodes_config = json.load(f)
                    # nodes_config的结构是 {"nodes": [...]}，需要提取nodes列表
                    nodes_list = nodes_config.get("nodes", [])
                    if not nodes_list:
                        logger.warning(f"⚠️  {wf_name} 的nodes_config中没有nodes字段或为空")
                        continue
                    logger.info(f"📊 找到 {len(nodes_list)} 个节点")
                    ...
                    processed_count += 1
                    logger.info(f"✅ 处理完成: {wf_name} (提取了 {len(variables_data.get('variables', {}))} 个变量)")
                except Exception as e:
                    logger.error(f"❌ 处理 {wf_name} 时出错: {e}")
                    raise
            else:
                logger.warning(f"⚠️  节点文件不存在: {nodes_file}")
                logger.warning(f"   工作流名称: {wf_name}")
                logger.warning(f"   目录: {step2_dir}")
                # 列出目录中的文件
                if os.path.exists(step2_dir):
                    files = os.listdir(step2_dir)
                    logger.warning(f"   目录中的文件: {files}")
                else:
                    logger.warning(f"   目录不存在: {step2_dir}")
                # 如果文件不存在，可能是路径问题，尝试查找其他可能的位置
                alt_step2_dir = os.path.join(output_dir, 'step2_workflow_config')
                if os.path.exists(alt_step2_dir):
                    logger.info(f"🔍 尝试查找替代目录: {alt_step2_dir}")
                    for subdir in os.listdir(alt_step2_dir):
                        ...
                        if os.path.isdir(subdir_path):
                            ...
                            if os.path.exists(alt_nodes_file):
                                logger.warning(f"⚠️  找到替代文件: {alt_nodes_file}")
                                break

        logger.info(f"✅ Step 4 完成: 处理了 {processed_count}/{len(generated_workflows)} 个 workflow")
        crud.update_step_status(db, task_id, 4, StepStatus.COMPLETED, f"变量提取完成 ({processed_count} 个文件)")
        
        # ========================================
        # Step 5: 提取配置
        # ========================================
        logger.info(f"🔄 开始 Step 5: 提取配置 ({len(generated_workflows)} 个 workflow)")
        ...
        for wf_name in generated_workflows:
            workflow_config = extract_workflow_config_from_flow(flow_data, request.file_path)
        ...
        for wf_name in generated_workflows:
            ...
            # 获取step2导出节点与边的配置文件，用于step6生成workflow
        ...
        project_name = os.path.basename(request.file_path).replace('exported_flow_', '').replace('.json', '')
        # 清理项目名称中的特殊字符，避免文件路径问题
        ...
        # 移除 agent_name 中的 exported_flow 前缀（如果存在）
        ...
        logger.debug(f"📁 生成的文件名: {os.path.basename(final_agent_file)}")
        logger.debug(f"   完整路径长度: {len(final_agent_file)} 字符")
        ...
        if lang_lower in ['en', 'english']:
            ...
            if os.path.exists(en_template):
                ...
                logger.debug(f"   使用英文 Agent 模板: {en_template}")
            else:
                logger.warning(f"   英文模板不存在: {en_template}，使用默认模板")
            ...
            if os.path.exists(zh_template):
                ...
                logger.debug(f"   使用简体中文 Agent 模板: {zh_template}")
            else:
                logger.warning(f"   简体中文模板不存在: {zh_template}，使用默认模板")
            ...
            if os.path.exists(zh_hant_template):
                ...
                logger.debug(f"   使用繁体中文/粤语 Agent 模板: {zh_hant_template}")
            else:
                logger.warning(f"   繁体中文模板不存在: {zh_hant_template}，使用默认模板")
                default_template = os.path.join('input', 'agent.json')
                if os.path.exists(default_template):
                    agent_template_path = default_template
        else:
            ...
            if os.path.exists(default_template):
                agent_template_path = default_template
                logger.debug(f"   使用默认 Agent 模板: {default_template}")
        
        # 确保 agent_template_path 不为 None
        if agent_template_path is None:
            logger.error(f"❌ 未找到合适的 Agent 模板文件 (语言: {lang})")
            logger.error(f"   请检查 input/ 目录下是否存在以下文件:")
            logger.error(f"   - agent-zh-hant.json (繁体中文)")
            logger.error(f"   - agent-zh.json (简体中文)")
            logger.error(f"   - agent_EN.json (英文)")
            logger.error(f"   - agent.json (默认)")
        ...
        if step7_files_list:
            merge_to_planning(
                ...
                exported_flow_file=request.file_path,
                task_output_dir=output_dir,  # 传入 task 输出目录
                global_config=global_config
            )
        
        # 更新 Agent 配置（仅在用户传入值时覆盖，默认值已在 agent.json 模板中定义）
        if os.path.exists(final_agent_file):
            ...
            if request.emb_language:
                ...
                if emb_lang_to_use:
                    ...
                    if old_emb != emb_lang_to_use:
                        basic_config['emb_language'] = emb_lang_to_use
                        logger.debug(f"  🔧 自动设置 emb_language: {old_emb} -> {emb_lang_to_use} (language={lang})")
            
            # 同步 knowledge.knowledge_base_list 中的 emb_language
            if emb_lang_to_use:
                ...
                if isinstance(kb_list, list) and kb_list:
                    ...
                    logger.debug(f"  🔧 已同步 knowledge_base_list 中的 emb_language 为: {emb_lang_to_use}")
            
            # 5. emb_model - 仅在传入时更新
            if request.emb_model:
                basic_config['emb_model'] = request.emb_model
                logger.debug(f"  🔧 更新 emb_model: {request.emb_model}")
            
            # 6. tip_message - 根据语言读取对应的 txt 文件
            ...
            if tip_message_file and os.path.exists(tip_message_file):
                try:
                    ...
                    logger.debug(f"  🔧 更新 tip_message: 从 {os.path.basename(tip_message_file)} 读取")
                except Exception as e:
                    logger.warning(f"  ⚠️ 读取 tip_message 文件失败: {e}")
            else:
                logger.warning(f"  ⚠️ tip_message 文件不存在: {tip_message_file}")
        ...
        if os.path.exists(kb_mapping_file) and os.path.exists(final_agent_file):
            ...
            if not os.path.exists(kb_mapping_file):
                logger.warning(f"⚠️ 知识库映射文件不存在，跳过Step 8.5: {kb_mapping_file}")
            if not os.path.exists(final_agent_file):
                logger.warning(f"⚠️ 最终文件不存在，跳过Step 8.5: {final_agent_file}")
        ...
        logger.info("="*80)
        crud.update_step_status(db, task_id, 9, StepStatus.RUNNING, "正在上传 Agent...")
        
        if os.path.exists(final_agent_file):
            logger.info(f"📁 Agent 文件: {os.path.basename(final_agent_file)}")
            from step9_upload_agent import upload_agent as upload_single_file
            ...
            success = upload_single_file(final_agent_file, api_config, dry_run=False)
            
            if success:
                logger.info("✅ Step 9 完成: Agent 上传成功！")
                logger.info("="*80)
                crud.update_step_status(db, task_id, 9, StepStatus.COMPLETED, "上传成功")
                result["success"] = True
            else:
                logger.error("❌ Step 9 失败: Agent 上传失败")
                logger.info("="*80)
                crud.update_step_status(db, task_id, 9, StepStatus.FAILED, "上传失败")
                result["error"] = "Agent 上传失败"
        else:
            logger.error(f"❌ Step 9 失败: Agent 文件不存在 - {final_agent_file}")
            logger.info("="*80)
            crud.update_step_status(db, task_id, 9, StepStatus.FAILED, "没有可上传的文件")
...
if __name__ == "__main__":
    ...
    logger.info("="*70)
    logger.info("🚀 Dialogflow CX 迁移工具 - FastAPI 服务")
    logger.info(f"📌 版本: v{version}")
    logger.info("="*70)
    logger.debug("")
    logger.debug("配置信息（从 .env 文件加载）:")
    logger.debug(f"  - 服务地址: {SERVER_HOST}:{SERVER_PORT}")
    logger.debug(f"  - 数据库: {os.getenv('DB_HOST', '192.168.160.58')}:{os.getenv('DB_PORT', '3306')}/{os.getenv('DB_NAME', 'cybertron_pre')}")
    logger.debug(f"  - Dyna API Base: {DYNA_API_BASE}")
    logger.debug(f"  - 回调地址: {CALLBACK_URL}")
